...
class Person(Thread):
    ...
    def __init__(self, id, n_items, goods, role, ns_name, hmac_key, haskey, save_output = False,output_location = ''):
        """
        ...
        try:
            ...
            with Pyro4.Daemon(host = self.hostname) as daemon:
                ...
                if self.role == "buyer":
                    ...
                    print("Average runtime for ", self.id, "was", str(avg_time))
                    print(runtimes)
                    print("Average runtime for ", self.id, "was", str(avg_time))
                #Seller loop
                while True:
                    ...
                    time.sleep(1)
                    if self.save_output:
                        myfile = open(self.output_location,'w')
                        for line in self.write_output:
                            myfile.write(line)
                            myfile.write('\n')
                        myfile.close() 
    ...
    def buy(self, peer_id):
        ...
        with self.itemlock:
            if self.n_items > 0:
                ...
                output = str(peer_id) + "purchased" + str(self.good) + "from" + str(self.id) + str(self.n_items) + "remains now" 
                self.write_output.append(output)
                return True
            # No more items to sell, randomly pick up another item
                ...
                self.write_output.append(str(peer_id) + "failed to purchase" + str(self.good))
                self.write_output.append(str(self.id) + "now sells" + str(self.n_items) + str(self.good))
                return False
